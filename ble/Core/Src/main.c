...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {//若需建立数据缓存区，见波特率动
	if (huart == &huart3) {
		...
		if (receiveData[0] == 0xAA) {
			if (receiveData[1] == Size) {
				...
				if (sum == receiveData[Size - 1]) {//[0]是第一位，[Size - 1]就是最后一位
					for (int i = 2; i < Size - 1; i += 2) { //每两个一组作为指令
		  ...
		  HAL_UARTEx_ReceiveToIdle_DMA(&huart3, receiveData, sizeof(receiveData));//串口不定长接收，注意DMA有过半中断
